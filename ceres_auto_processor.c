...
int main(int argc, char *argv[]) {
    ...
    char      avg_file_start[] = "/home/bsorenson/HighLatitudeStudy/CERES/average_files/TERRA/temp_clr_files/ceres_avg_gridded_flux_";
        ...
        if(slen>2)
        {
            ...
            if(strcmp(input_date,check_date)==0)
            {
                ...
                if(strcmp(met_check,"met")!=0)
                {
                    ...
                    readhdfdim(temp_path,"Clear area percent coverage at subpixel resolution", &nelement);
                    clr = (float *) malloc( nelement * sizeof(float));
                    readhdf(temp_path,"Clear area percent coverage at subpixel resolution", &llsize0,  &gain, &ofst, clr);
