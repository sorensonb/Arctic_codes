...
!  ############################################################################
  ...
  integer(hsize_t), dimension(:), allocatable       :: TROP_LAT_dims
  integer(hsize_t), dimension(:), allocatable       :: TROP_LON_dims
  ...
  real(kind=8), dimension(:),   allocatable, target :: TROP_LAT_data
  real(kind=8), dimension(:),   allocatable, target :: TROP_LON_data
  ...
  contains
    ...
    ! Dimensions for output data
    ! --------------------------
    allocate(TROP_out_AI_data(OMI_AI_dims(1), OMI_AI_dims(2)), stat = error)
    ...
    ! -------------------------------------------------------------
      ...
      if(allocated(TROP_LAT_dims))    deallocate(TROP_LAT_dims)
      if(allocated(TROP_LON_dims))    deallocate(TROP_LON_dims)
      ...
      if(allocated(TROP_LAT_data))    deallocate(TROP_LAT_data)
      if(allocated(TROP_LON_data))    deallocate(TROP_LON_data)
