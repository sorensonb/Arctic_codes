subroutine read_comp_OMI_LATCRNR(file_id)
!
! NAME:
!   read_h5_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only : OMI_LATCRNR_dims, OMI_LATCRNR_data
  ...
  ! Open OMI_LATCRNR dataset
  ! ----------------
  call h5dopen_f(file_id, 'lat_crnr', ds_id, error)
  if(error /= 0) then
  ...
  ! Determine the number of dimensions in the dataset, allocate the OMI_LATCRNR_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(OMI_LATCRNR_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the OMI_LATCRNR_dims array
  do ii=1,ndims
    OMI_LATCRNR_dims(ii) = datadims(ii)
  enddo
 
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(OMI_LATCRNR_data(OMI_LATCRNR_dims(1), OMI_LATCRNR_dims(2), &
            OMI_LATCRNR_dims(3)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, OMI_LATCRNR_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_comp_OMI_LATCRNR
