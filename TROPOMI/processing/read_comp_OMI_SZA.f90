subroutine read_comp_OMI_SZA(file_id)
!
! NAME:
!   read_h5_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only : OMI_SZA_dims, OMI_SZA_data
  ...
  ! ----------------
  call h5dopen_f(file_id, 'sza', ds_id, error)
  if(error /= 0) then
...
end subroutine read_comp_OMI_SZA
