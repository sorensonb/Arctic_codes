subroutine grid_raw_data_daily_climo(i_day_idx, i_size,lat_gridder, &
        lat_thresh)
!
!  NAME:
...
!
!  MODIFICATIONS:
!    Blake Sorenson <blake.sorenson@und.edu>     - 2023/09/11: Written
!
!  ###########################################################################
    ...
    !row_loop: do jj=1,AI_dims(1) 
      ...
      if((XTRACK_data(jj,ii) == 0) .and. &
        ...
        grid_AI(index2,index1,i_day_idx) = &
            grid_AI(index2,index1,i_day_idx) + AI_data(jj,ii)
...
end subroutine grid_raw_data_daily_climo
