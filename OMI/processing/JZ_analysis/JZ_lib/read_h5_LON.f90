...
!  ############################################################################
  ...
  use hdf5
  use h5_vars, only : LON_dims, LON_data
  ...
  ! Determine the number of dimensions in the dataset, allocate the LON_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(LON_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the LON_dims array
  do ii=1,ndims
    LON_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(LON_data(LON_dims(1), LON_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, LON_data, dims, &
                 error)
  if (error.lt.0) then
