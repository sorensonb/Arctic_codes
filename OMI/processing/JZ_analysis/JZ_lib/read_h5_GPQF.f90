subroutine read_h5_GPQF(file_id)
!
! NAME:
!   read_h5_GPQF
!
! PURPOSE:
!   Read GroundPixelQualityFlags data from an HDF5 file pointed to by file_id 
!   and store the data in GPQF_data (and the dimensions in GPQF_dims) from
!   h5_vars.
!  
! CALLS:
!   Modules:
...
!  ############################################################################
  ...
  use hdf5
  use h5_vars, only : GPQF_dims, GPQF_data
  ...
  ! Determine the number of dimensions in the dataset, allocate the GPQF_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(GPQF_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the GPQF_dims array
  do ii=1,ndims
    GPQF_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(GPQF_data(GPQF_dims(1), GPQF_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_INTEGER, GPQF_data, dims, &
                 error)
  if (error.lt.0) then
