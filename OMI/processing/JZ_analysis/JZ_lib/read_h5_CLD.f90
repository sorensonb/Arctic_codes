subroutine read_h5_CLD(file_id)
!
! NAME:
!   read_h5_CLD
!
...
!   and store the data in CLD_data (and the dimensions in CLD_dims) from
!   h5_vars.
! 
! CALLS:
!   Modules:
...
!  ############################################################################
  ...
  use hdf5
  use h5_vars, only : CLD_dims, CLD_data
  ...
  ! Open CLD dataset
  ! ----------------
  call h5dopen_f(file_id, 'HDFEOS/SWATHS/Aerosol NearUV Swath/Data'&
        //' Fields/CloudFraction', ds_id, error)
  if(error /= 0) then
  ...
  ! Determine the number of dimensions in the dataset, allocate the CLD_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(CLD_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the CLD_dims array
  do ii=1,ndims
    CLD_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(CLD_data(CLD_dims(1), CLD_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, CLD_data, dims, &
                 error)
  if (error.lt.0) then
