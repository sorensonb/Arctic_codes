subroutine read_h5_LATCRNR(file_id)
!
! NAME:
!   read_h5_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use h5_vars, only : LATCRNR_dims, LATCRNR_data
  ...
  ! Determine the number of dimensions in the dataset, allocate the LATCRNR_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(LATCRNR_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the LATCRNR_dims array
  do ii=1,ndims
    LATCRNR_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(LATCRNR_data(LATCRNR_dims(1), LATCRNR_dims(2), LATCRNR_dims(3)), &
    stat=error)
  if ( error < 0 ) then
  ...
  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, LATCRNR_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_h5_LATCRNR
