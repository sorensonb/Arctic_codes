subroutine read_h5_SZA(file_id)
!
! NAME:
!   read_h5_SZA
!
...
!   and store the data in SZA_data (and the dimensions in SZA_dims) from
!   h5_vars.
! 
! CALLS:
!   Modules:
...
!  ############################################################################
  ...
  use hdf5
  use h5_vars, only : SZA_dims, SZA_data
  ...
  call h5dopen_f(file_id, 'HDFEOS/SWATHS/Aerosol NearUV Swath/Geolocation'&
        //' Fields/SolarZenithAngle', ds_id, error)
  if(error /= 0) then
  ...
  ! Determine the number of dimensions in the dataset, allocate the SZA_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(SZA_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the SZA_dims array
  do ii=1,ndims
    SZA_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(SZA_data(SZA_dims(1), SZA_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, SZA_data, dims, &
                 error)
  if (error.lt.0) then
