...
if(len(total_list) > 1):
    ...
    g_GPQF = np.full(GPQF_decode.shape,-9)
    g_GPQF[(GPQF_decode > 0) & (GPQF_decode < 101)] = 9
    g_GPQF[GPQF_decode == 0]                        = 8
    g_GPQF[GPQF_decode == 101]                      = 10
    g_GPQF[GPQF_decode == 103]                      = 11
    g_GPQF[GPQF_decode == 104]                      = 12
    g_GPQF[GPQF_decode == 124]                      = 13

    mask_UVAI = np.ma.masked_where(g_GPQF == -9, g_GPQF)
    plot_lat = LAT_flat
    plot_lon = LON_flat
