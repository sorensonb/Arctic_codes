...
!  ############################################################################
        ...
        omi_loop1: do ii = 1, OMI_AI_dims(2)
          ...
          omi_loop2: do jj = 1, OMI_AI_dims(1)
            ...
            if( ( (OMI_AI_data(jj,ii) > 2) .and. &
              ...
              !!#!    sumxy_values(cld_idx,  sza_idx, ice_idx) + &
              !!#!    (CERES_SWF_data(jj,ii) * OMI_AI_data(jj,ii))
  ...
  write(*,*) 'Max identified COD:', max_found_cod
      ...
      do nn = 1, len_cld
        do mm = 1, len_cod
          ...
          if(count_values(mm,nn,jj,ii) > 1) then
            meanx_values(mm,nn,jj,ii) = sumx_values(mm,nn,jj,ii) / count_values(mm,nn,jj,ii)
            meany_values(mm,nn,jj,ii) = sumy_values(mm,nn,jj,ii) / count_values(mm,nn,jj,ii)


            s_xy(mm,nn,jj,ii) = sumxy_values(mm,nn,jj,ii) - &
                ( (sumx_values(mm,nn,jj,ii) * sumy_values(mm,nn,jj,ii)) / &
                count_values(mm,nn,jj,ii))
            s_xx(mm,nn,jj,ii) = sumx2_values(mm,nn,jj,ii) - &
                ( (sumx_values(mm,nn,jj,ii)**2.) / count_values(mm,nn,jj,ii) ) 

            regress_slopes(mm,nn,jj,ii) = s_xy(mm,nn,jj,ii) / s_xx(mm,nn,jj,ii)

            regress_intercepts(mm,nn,jj,ii) = meany_values(mm,nn,jj,ii) - &
                regress_slopes(mm,nn,jj,ii) * meanx_values(mm,nn,jj,ii)

            sse(mm,nn,jj,ii) = sumy2_values(mm,nn,jj,ii) - &
                regress_intercepts(mm,nn,jj,ii) * sumy_values(mm,nn,jj,ii) - &
                regress_slopes(mm,nn,jj,ii) * sumxy_values(mm,nn,jj,ii)

            sst(mm,nn,jj,ii) = sumy2_values(mm,nn,jj,ii) - &
                (sumy_values(mm,nn,jj,ii)**2.) / count_values(mm,nn,jj,ii)

            regress_r2(mm,nn,jj,ii) = 1. - (sse(mm,nn,jj,ii) / sst(mm,nn,jj,ii))
 
            regress_slope_error(mm,nn,jj,ii) = ( (1. / &
                (count_values(mm,nn,jj,ii) - 2)) * &
                ( sse(mm,nn,jj,ii) / s_xx(mm,nn,jj,ii)  )  ) ** 0.5

            if(  (nn == check_cld_idx) .and. (jj == check_sza_idx) .and. &
                 (ii == check_ice_idx) .and. (mm == check_cod_idx) ) then
              write(*,*) 'CHECKING THE FOLLOWING'
            endif
            write(*,'(f7.1,1x,f5.1,1x,i3,1x,f6.2,i7,1x,f9.2,2x,f7.2)') ice_bins(ii),sza_bins(jj),nn - 1,cod_bins(mm), &
                count_values(mm,nn,jj,ii), regress_slopes(mm,nn,jj,ii), regress_r2(mm,nn,jj,ii)
