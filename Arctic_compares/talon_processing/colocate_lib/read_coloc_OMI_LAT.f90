subroutine read_coloc_OMI_LAT(file_id)
!
! NAME:
!   read_coloc_OMI_LAT
!
...
!   and store the data in OMI_LAT_data (and the dimensions in OMI_LAT_dims) from
!   h5_vars.
!  
! CALLS:
!   Modules:
...
!  ############################################################################
  ...
  use hdf5
  use comp_grid_vars, only : OMI_LAT_dims, OMI_LAT_data
  ...
  ! Open AI dataset
  ! ---------------
  call h5dopen_f(file_id, 'omi_lat', ds_id, error)
  if(error /= 0) then
  ...
  ! Determine the number of dimensions in the dataset, allocate the OMI_LAT_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(OMI_LAT_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the OMI_LAT_dims array
  do ii=1,ndims
    OMI_LAT_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(OMI_LAT_data(OMI_LAT_dims(1), OMI_LAT_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, OMI_LAT_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_coloc_OMI_LAT
