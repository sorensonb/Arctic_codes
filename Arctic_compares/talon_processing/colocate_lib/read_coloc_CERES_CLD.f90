subroutine read_coloc_CERES_CLD(file_id)
!
! NAME:
!   read_coloc_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use comp_grid_vars, only : CERES_CLD_dims, CERES_CLD_data
  ...
  call h5dopen_f(file_id, 'ceres_cld', ds_id, error)
  ...
  allocate(CERES_CLD_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  do ii=1,ndims
    CERES_CLD_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(CERES_CLD_data(CERES_CLD_dims(1), CERES_CLD_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, CERES_CLD_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_coloc_CERES_CLD
