subroutine read_coloc_OMI_SZA(file_id)
!
! NAME:
!   read_h5_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use comp_grid_vars, only : OMI_SZA_dims, OMI_SZA_data
  ...
  ! Open OMI_SZA dataset
  ! ----------------
  call h5dopen_f(file_id, 'omi_sza', ds_id, error)
  if(error /= 0) then
  ...
  ! Determine the number of dimensions in the dataset, allocate the OMI_SZA_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(OMI_SZA_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the OMI_SZA_dims array
  do ii=1,ndims
    OMI_SZA_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(OMI_SZA_data(OMI_SZA_dims(1), OMI_SZA_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, OMI_SZA_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_coloc_OMI_SZA
