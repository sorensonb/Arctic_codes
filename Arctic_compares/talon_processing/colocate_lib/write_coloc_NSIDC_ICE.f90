subroutine write_coloc_NSIDC_ICE(out_file_id)
!
! NAME:
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only: NSIDC_out_data,    OMI_AI_dims
  ...
  ! Create the dataset
  ! ------------------
  call h5dcreate_f(out_file_id, 'nsidc_ice', H5T_NATIVE_DOUBLE, dspace_id,  &
                   dset_id, error) 
  if(error /= 0) then
  ...
  ! --------------------
  call h5dwrite_f(dset_id, H5T_NATIVE_DOUBLE, NSIDC_out_data, OMI_AI_dims, &
                      error)
  if(error /= 0) then
...
end subroutine write_coloc_NSIDC_ICE
