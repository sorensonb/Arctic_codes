subroutine read_coloc_MODIS_CH7(file_id)
!
! NAME:
!   read_coloc_MODIS_CH7
!
! PURPOSE:
!   Read Latitude data from an HDF5 file pointed to by file_id 
!   and store the data in AI_CH7 (and the dimensions in AI_dims) from
!   h5_vars.
!  
! CALLS:
!   Modules:
...
!  ############################################################################
  ...
  use hdf5
  use comp_grid_vars, only : MODIS_CH7_dims, MODIS_CH7_data
  ...
  call h5dopen_f(file_id, 'modis_ch7', ds_id, error)
  ...
  allocate(MODIS_CH7_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  do ii=1,ndims
    MODIS_CH7_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(MODIS_CH7_data(MODIS_CH7_dims(1), MODIS_CH7_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, MODIS_CH7_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_coloc_MODIS_CH7
