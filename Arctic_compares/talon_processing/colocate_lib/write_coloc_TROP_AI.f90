subroutine write_coloc_TROP_AI(out_file_id)
!
! NAME:
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only: TROP_out_AI_data,    OMI_AI_dims
  ...
  ! --------------------
  call h5dwrite_f(dset_id, H5T_NATIVE_DOUBLE, TROP_out_AI_data, &
                  OMI_AI_dims, error)
  if(error /= 0) then
...
end subroutine write_coloc_TROP_AI
