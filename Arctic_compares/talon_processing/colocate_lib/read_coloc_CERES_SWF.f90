subroutine read_coloc_CERES_SWF(file_id)
!
! NAME:
!   read_coloc_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use comp_grid_vars, only : CERES_SWF_dims, CERES_SWF_data
  ...
  call h5dopen_f(file_id, 'ceres_swf', ds_id, error)
  ...
  ! Determine the number of dimensions in the dataset, allocate the SWF_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(CERES_SWF_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the SWF_dims array
  do ii=1,ndims
    CERES_SWF_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(CERES_SWF_data(CERES_SWF_dims(1), CERES_SWF_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, CERES_SWF_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_coloc_CERES_SWF
