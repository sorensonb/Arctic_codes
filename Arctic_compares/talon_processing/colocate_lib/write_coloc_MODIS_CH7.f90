subroutine write_coloc_MODIS_CH7(out_file_id)
!
! NAME:
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only: MODIS_out_CH7_data,    OMI_AI_dims
  ...
  ! Create the dataset
  ! ------------------
  call h5dcreate_f(out_file_id, 'modis_ch7', H5T_NATIVE_DOUBLE, dspace_id,  &
                   dset_id, error) 
  if(error /= 0) then
  ...
  ! --------------------
  call h5dwrite_f(dset_id, H5T_NATIVE_DOUBLE, MODIS_out_CH7_data, OMI_AI_dims, &
                      error)
  if(error /= 0) then
...
end subroutine write_coloc_MODIS_CH7
