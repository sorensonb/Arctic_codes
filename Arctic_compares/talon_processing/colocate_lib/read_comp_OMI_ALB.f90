subroutine read_comp_OMI_ALB(file_id)
!
! NAME:
!   read_h5_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only : OMI_ALB_dims, OMI_ALB_data
  ...
  ! Open OMI_ALB dataset
  ! ----------------
  call h5dopen_f(file_id, 'alb', ds_id, error)
  if(error /= 0) then
  ...
  ! Determine the number of dimensions in the dataset, allocate the OMI_ALB_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(OMI_ALB_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the OMI_ALB_dims array
  do ii=1,ndims
    OMI_ALB_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(OMI_ALB_data(OMI_ALB_dims(1), OMI_ALB_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, OMI_ALB_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_comp_OMI_ALB
