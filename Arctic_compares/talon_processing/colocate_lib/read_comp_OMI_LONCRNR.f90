subroutine read_comp_OMI_LONCRNR(file_id)
!
! NAME:
!   read_h5_OMI_LONCRNR
!
...
!   and store the data in OMI_LONCRNR_data (and the dimensions in OMI_LONCRNR_dims) from
!   h5_vars.
!  
! CALLS:
!   Modules:
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only : OMI_LONCRNR_dims, OMI_LONCRNR_data
  ...
  ! Open OMI_LONCRNR dataset
  ! ----------------
  call h5dopen_f(file_id, 'lon_crnr', ds_id, error)
  if(error /= 0) then
  ...
  ! Determine the number of dimensions in the dataset, allocate the OMI_LONCRNR_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(OMI_LONCRNR_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the OMI_LONCRNR_dims array
  do ii=1,ndims
    OMI_LONCRNR_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(OMI_LONCRNR_data(OMI_LONCRNR_dims(1), OMI_LONCRNR_dims(2), &
            OMI_LONCRNR_dims(3)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, OMI_LONCRNR_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_comp_OMI_LONCRNR
