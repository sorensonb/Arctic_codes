subroutine read_comp_CERES_ALB(file_id)
!
! NAME:
!   read_comp_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only : CERES_ALB_dims, CERES_ALB_data
  ...
  ! Determine the number of dimensions in the dataset, allocate the ALB_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(CERES_ALB_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the ALB_dims array
  do ii=1,ndims
    CERES_ALB_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(CERES_ALB_data(CERES_ALB_dims(1), CERES_ALB_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, CERES_ALB_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_comp_CERES_ALB
