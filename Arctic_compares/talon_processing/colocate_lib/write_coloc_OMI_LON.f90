subroutine write_coloc_OMI_LON(out_file_id)
!
! NAME:
!   write_grid_TROP_LON.f90
!
! PURPOSE:
! 
! CALLS:
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only: OMI_LON_data,    OMI_AI_dims
  ...
  ! Create the dataset
  ! ------------------
  call h5dcreate_f(out_file_id, 'omi_lon', H5T_NATIVE_DOUBLE, dspace_id,  &
                   dset_id, error) 
  if(error /= 0) then
  ...
  ! --------------------
  call h5dwrite_f(dset_id, H5T_NATIVE_DOUBLE, OMI_LON_data, OMI_AI_dims, &
                      error)
  if(error /= 0) then
...
end subroutine write_coloc_OMI_LON
