subroutine read_coloc_TROP_AI(file_id)
!
! NAME:
!   read_coloc_TROP_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use comp_grid_vars, only : TROP_AI_dims, TROP_AI_data
  ...
  ! Open data dataset
  ! ----------------
  call h5dopen_f(file_id, 'trop_uvai', ds_id, error)
  if(error /= 0) then
  ...
  allocate(TROP_AI_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  !TROP_AI_dims(1) = datadims(1)
  do ii=1,ndims
    TROP_AI_dims(ii) = datadims(ii)
  enddo

  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(TROP_AI_data(TROP_AI_dims(1), TROP_AI_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, TROP_AI_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_coloc_TROP_AI
