subroutine read_comp_OMI_AI_raw(file_id)
!
! NAME:
!   read_comp_OMI_AI_raw
!
! PURPOSE:
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only : OMI_AI_raw_dims, OMI_AI_raw_data
  ...
  ! Open AI dataset
  ! ---------------
  call h5dopen_f(file_id, 'uvai_raw', ds_id, error)
  if(error /= 0) then
...
end subroutine read_comp_OMI_AI_raw
