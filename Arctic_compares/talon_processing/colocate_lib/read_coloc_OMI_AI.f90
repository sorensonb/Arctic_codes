subroutine read_coloc_OMI_AI(file_id)
!
! NAME:
!   read_coloc_OMI_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use comp_grid_vars, only : OMI_AI_dims, OMI_AI_data
  ...
  ! Open AI dataset
  ! ---------------
  call h5dopen_f(file_id, 'omi_uvai_pert', ds_id, error)
  if(error /= 0) then
...
end subroutine read_coloc_OMI_AI
