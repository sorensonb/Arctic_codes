subroutine read_comp_TROP_LON(file_id)
!
! NAME:
!   read_comp_TROP_LON
!
...
!   and store the data in TROP_LON_data (and the dimensions in TROP_LON_dims) from
!   h5_vars.
!  
! CALLS:
!   Modules:
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only : TROP_LON_dims, TROP_LON_data
  ...
  ! Determine the number of dimensions in the dataset, allocate the TROP_LON_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(TROP_LON_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the TROP_LON_dims array
  !TROP_LON_dims(1) = datadims(1)
  do ii=1,ndims
    TROP_LON_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(TROP_LON_data(TROP_LON_dims(1)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, TROP_LON_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_comp_TROP_LON
