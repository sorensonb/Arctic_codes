subroutine write_coloc_MODIS_CTP(out_file_id)
!
! NAME:
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only: MODIS_out_CTP_data,    OMI_AI_dims
  ...
  ! Create the dataset
  ! ------------------
  call h5dcreate_f(out_file_id, 'modis_cld_top_pres', H5T_NATIVE_DOUBLE, dspace_id,  &
                   dset_id, error) 
  if(error /= 0) then
    write(*,*) 'FATAL ERROR: could not open dataset '//'modis_ctp'
    return
  endif

  ! Write to the dataset
  ! --------------------
  call h5dwrite_f(dset_id, H5T_NATIVE_DOUBLE, MODIS_out_CTP_data, OMI_AI_dims, &
                      error)
  if(error /= 0) then
...
end subroutine write_coloc_MODIS_CTP
