subroutine write_coloc_CERES_LWF(out_file_id)
!
! NAME:
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only: CERES_LWF_data,    OMI_AI_dims
  ...
  ! Create the dataset
  ! ------------------
  call h5dcreate_f(out_file_id, 'ceres_lwf', H5T_NATIVE_DOUBLE, dspace_id,  &
                   dset_id, error) 
  if(error /= 0) then
  ...
  ! --------------------
  call h5dwrite_f(dset_id, H5T_NATIVE_DOUBLE, CERES_LWF_data, OMI_AI_dims, &
                      error)
  if(error /= 0) then
...
end subroutine write_coloc_CERES_LWF
