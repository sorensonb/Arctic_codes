subroutine read_comp_MODIS_CTP(file_id)
!
! NAME:
!   read_comp_MODIS_CTP
!
...
!   and store the data in AI_CTP (and the dimensions in AI_dims) from
!   h5_vars.
!  
! CALLS:
!   Modules:
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only : MODIS_CTP_dims, MODIS_CTP_data
  ...
  call h5dopen_f(file_id, 'cld_top_pres', ds_id, error)
  ...
  allocate(MODIS_CTP_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  do ii=1,ndims
    MODIS_CTP_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(MODIS_CTP_data(MODIS_CTP_dims(1), MODIS_CTP_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, MODIS_CTP_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_comp_MODIS_CTP
