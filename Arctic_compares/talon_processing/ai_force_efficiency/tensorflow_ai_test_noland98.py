...
if(l_load_model):
    ...
    l_USE_ONLY_PRE_NOLAND97_FILES = True
    ...
    if(l_USE_ONLY_PRE_NOLAND56_FILES):
        ...
        base_files = glob('/home/blake.sorenson/OMI/arctic_comp/comp_data/colocated*.hdf5')
    ...
    # aerosol swaths
    # ---------------------------------------------------------------------
    int_list = [int(tdate[:8]) for tdate in file_list]
    keep_date_list = np.unique(np.concatenate(np.array([np.arange(idate - 4, \
        idate + 5) for idate in int_list])))
    ...
    combined_data['nsidc_ice'][:] = \
        np.where(combined_data['nsidc_ice'][:] == 253., 100.5, combined_data['nsidc_ice'][:])
    combined_data['nsidc_ice'][:] = \
...
if(l_save_data):
    ...
    #for infile in clear_file_list:
        ...
        data_mask = np.ma.masked_where( \
                    ...
                    (np.isnan(data['modis_cld_top_pres'][:,:]) == True) | \
                    (data['nsidc_ice'][:,:] == -999) | \
                    (data['nsidc_ice'][:,:] == 251), data['omi_lat'])
        ...
        # NEW TO noland98
        # ---------------
        local_ice     = np.where(data['nsidc_ice'][:,:] == 253.,100.5, \
                                    data['nsidc_ice'])
