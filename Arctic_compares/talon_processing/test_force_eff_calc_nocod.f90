...
!  ############################################################################
  ...
  integer(hsize_t), dimension(:)                   :: regress_dims(3)
  ...
  real, dimension(:,:,:), allocatable                  :: meanx_values
  real, dimension(:,:,:), allocatable                  :: meany_values

  !!#!real, dimension(:,:,:), allocatable                  :: sum_pred_err_square
  !!#!real, dimension(:,:,:), allocatable                  :: sum_x_pert_square

  real, dimension(:,:,:), allocatable                  :: s_xy
  real, dimension(:,:,:), allocatable                  :: s_xx
  real, dimension(:,:,:), allocatable                  :: sse 
  ...
  allocate(sumx_values(len_cld, len_sza, len_ice))
  allocate(sumy_values(len_cld, len_sza, len_ice))
  allocate(sumx2_values(len_cld, len_sza, len_ice))
  allocate(sumy2_values(len_cld, len_sza, len_ice))
  allocate(sumxy_values(len_cld, len_sza, len_ice))
  allocate(count_values(len_cld, len_sza, len_ice))
  ...
  ! standard error
  allocate(meanx_values(len_cld, len_sza, len_ice))
  allocate(meany_values(len_cld, len_sza, len_ice))
  allocate(s_xy(len_cld, len_sza, len_ice))
  allocate(s_xx(len_cld, len_sza, len_ice))
  allocate(sse(len_cld, len_sza, len_ice))
  allocate(sst(len_cld, len_sza, len_ice))
  ...
  allocate(regress_slopes(len_cld, len_sza, len_ice))
  allocate(regress_intercepts(len_cld, len_sza, len_ice))
  allocate(regress_slope_error(len_cld, len_sza, len_ice))
  allocate(regress_r2(len_cld, len_sza, len_ice))
  ...
  io7 = 2007
  open(io7, file = 'test_out_file_nocod.txt', iostat = istatus)
  if(istatus > 0) then
    write(*,*) 'ERROR: Problem opening test_out_file_nocod.txt'
  endif
        ...
        omi_loop1: do ii = 1, OMI_AI_dims(2)
          ...
          omi_loop2: do jj = 1, OMI_AI_dims(1)
            ...
            if( ( (OMI_AI_data(jj,ii) > 2) .and. &
              ...
              sumx_values(cld_idx, sza_idx, ice_idx) = &
                sumx_values(cld_idx, sza_idx, ice_idx) + OMI_AI_data(jj,ii)
              sumy_values(cld_idx, sza_idx, ice_idx) = &
                sumy_values(cld_idx, sza_idx, ice_idx) + CERES_SWF_data(jj,ii)
              sumx2_values(cld_idx, sza_idx, ice_idx) = &
                sumx2_values(cld_idx, sza_idx, ice_idx) + OMI_AI_data(jj,ii)**2.
              sumy2_values(cld_idx, sza_idx, ice_idx) = &
                sumy2_values(cld_idx, sza_idx, ice_idx) + CERES_SWF_data(jj,ii)**2.
              sumxy_values(cld_idx, sza_idx, ice_idx) = &
                sumxy_values(cld_idx, sza_idx, ice_idx) + &
                (OMI_AI_data(jj,ii) * CERES_SWF_data(jj,ii))
              count_values(cld_idx, sza_idx, ice_idx) = &
                count_values(cld_idx, sza_idx, ice_idx) + 1
  ...
  do ii = 1, len_ice
    do jj = 1, len_sza
      do nn = 1, len_cld
        ...
        ! --------------------------------------------
        if(count_values(nn,jj,ii) > 1) then
          meanx_values(nn,jj,ii) = sumx_values(nn,jj,ii) / count_values(nn,jj,ii)
          meany_values(nn,jj,ii) = sumy_values(nn,jj,ii) / count_values(nn,jj,ii)


          s_xy(nn,jj,ii) = sumxy_values(nn,jj,ii) - &
              ( (sumx_values(nn,jj,ii) * sumy_values(nn,jj,ii)) / &
              count_values(nn,jj,ii))
          s_xx(nn,jj,ii) = sumx2_values(nn,jj,ii) - &
              ( (sumx_values(nn,jj,ii)**2.) / count_values(nn,jj,ii) ) 

          regress_slopes(nn,jj,ii) = s_xy(nn,jj,ii) / s_xx(nn,jj,ii)

          regress_intercepts(nn,jj,ii) = meany_values(nn,jj,ii) - &
              regress_slopes(nn,jj,ii) * meanx_values(nn,jj,ii)

          sse(nn,jj,ii) = sumy2_values(nn,jj,ii) - &
              regress_intercepts(nn,jj,ii) * sumy_values(nn,jj,ii) - &
              regress_slopes(nn,jj,ii) * sumxy_values(nn,jj,ii)

          sst(nn,jj,ii) = sumy2_values(nn,jj,ii) - &
              (sumy_values(nn,jj,ii)**2.) / count_values(nn,jj,ii)

          regress_r2(nn,jj,ii) = 1. - (sse(nn,jj,ii) / sst(nn,jj,ii))
 
          regress_slope_error(nn,jj,ii) = ( (1. / &
              (count_values(nn,jj,ii) - 2)) * &
              ( sse(nn,jj,ii) / s_xx(nn,jj,ii)  )  ) ** 0.5

          if(  (nn == check_cld_idx) .and. (jj == check_sza_idx) .and. &
               (ii == check_ice_idx) ) then
            write(*,*) 'CHECKING THE FOLLOWING'
          endif
          write(*,'(3(i4),f7.1,1x,f5.1,1x,i3,1x,i7,1x,f9.2,2x,f7.2,2x,f7.2)') &
              nn, jj, ii, &
              ice_bins(ii),sza_bins(jj),nn - 1, &
              count_values(nn,jj,ii), regress_slopes(nn,jj,ii), &
              regress_slope_error(nn,jj,ii), regress_r2(nn,jj,ii)
  ...
  !
  ! = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
  out_file_name = 'force_effic_values_nocod.h5'
  ...
  deallocate(sse)
  deallocate(sst)
  deallocate(meanx_values)
  deallocate(meany_values)
  !!#!deallocate(sum_pred_err_square)
  !!#!deallocate(sum_x_pert_square)
  deallocate(regress_slopes)
  deallocate(regress_intercepts)
  deallocate(regress_slope_error)
  deallocate(regress_r2)

  deallocate(sumx_values)
  deallocate(sumy_values)
  deallocate(sumx2_values)
  deallocate(sumy2_values)
  deallocate(sumxy_values)
