subroutine read_coloc_OMI_LON(file_id)
!
! NAME:
!   read_coloc_OMI_LON
!
...
!  ############################################################################
  ...
  use hdf5
  use comp_grid_vars, only : OMI_LON_dims, OMI_LON_data
  ...
  ! Open AI dataset
  ! ---------------
  call h5dopen_f(file_id, 'omi_lon', ds_id, error)
  if(error /= 0) then
...
end subroutine read_coloc_OMI_LON
