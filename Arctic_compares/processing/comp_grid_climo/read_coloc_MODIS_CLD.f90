subroutine read_coloc_MODIS_CLD(file_id)
!
! NAME:
!   read_coloc_MODIS_CLD
!
! PURPOSE:
!   Read Latitude data from an HDF5 file pointed to by file_id 
!   and store the data in AI_CLD (and the dimensions in AI_dims) from
!   h5_vars.
!  
! CALLS:
!   Modules:
...
!  ############################################################################
  ...
  use hdf5
  use comp_grid_vars, only : MODIS_CLD_dims, MODIS_CLD_data
  ...
  call h5dopen_f(file_id, 'modis_cld', ds_id, error)
  ...
  allocate(MODIS_CLD_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  do ii=1,ndims
    MODIS_CLD_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(MODIS_CLD_data(MODIS_CLD_dims(1), MODIS_CLD_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, MODIS_CLD_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_coloc_MODIS_CLD
