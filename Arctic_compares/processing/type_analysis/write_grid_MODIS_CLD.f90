subroutine write_grid_MODIS_CLD(out_file_id, grid_data_dims)
!
! NAME:
!   write_grid_MODIS_CLD.f90
!
! PURPOSE:
! 
! CALLS:
...
!  ############################################################################
  ...
  ! Create the dataset
  ! ------------------
  call h5dcreate_f(out_file_id, 'modis_cld', H5T_NATIVE_REAL, &
  ...
  ! --------------------
  call h5dwrite_f(dset_id, H5T_NATIVE_REAL, grid_MODIS_CLD, grid_data_dims, &
                      error)
  if(error /= 0) then
  ...
  !!#!! Write to the dataset
  !!#!! --------------------
  !!#!call h5dwrite_f(dset_id, H5T_NATIVE_INTEGER, count_MODIS_CLD, grid_data_dims, &
  !!#!                    error)
  !!#!if(error /= 0) then
  !!#!  write(*,*) 'FATAL ERROR: could not write to dataset'
  !!#!  return
...
end subroutine write_grid_MODIS_CLD
