subroutine read_comp_OMI_LON(file_id)
!
! NAME:
!   read_h5_OMI_LON
!
...
!   and store the data in OMI_LON_data (and the dimensions in OMI_LON_dims) from
!   h5_vars.
!  
! CALLS:
!   Modules:
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only : OMI_LON_dims, OMI_LON_data
  ...
  ! Determine the number of dimensions in the dataset, allocate the OMI_LON_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(OMI_LON_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the OMI_LON_dims array
  do ii=1,ndims
    OMI_LON_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(OMI_LON_data(OMI_LON_dims(1), OMI_LON_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, OMI_LON_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_comp_OMI_LON
