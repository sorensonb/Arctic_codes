subroutine read_comp_MODIS_CH1(file_id)
!
! NAME:
!   read_comp_MODIS_CH1
!
...
!   and store the data in AI_CH1 (and the dimensions in AI_dims) from
!   h5_vars.
!  
! CALLS:
!   Modules:
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only : MODIS_CH1_dims, MODIS_CH1_data
  ...
  allocate(MODIS_CH1_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  do ii=1,ndims
    MODIS_CH1_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(MODIS_CH1_data(MODIS_CH1_dims(1), MODIS_CH1_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, MODIS_CH1_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_comp_MODIS_CH1
