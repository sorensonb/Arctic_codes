subroutine read_comp_TROP_LAT(file_id)
!
! NAME:
!   read_comp_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only : TROP_LAT_dims, TROP_LAT_data
  ...
  allocate(TROP_LAT_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  !TROP_LAT_dims(1) = datadims(1)
  do ii=1,ndims
    TROP_LAT_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(TROP_LAT_data(TROP_LAT_dims(1)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, TROP_LAT_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_comp_TROP_LAT
