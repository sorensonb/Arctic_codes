subroutine read_comp_CERES_LAT(file_id)
!
! NAME:
!   read_comp_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only : CERES_LAT_dims, CERES_LAT_data
  ...
  allocate(CERES_LAT_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  do ii=1,ndims
    CERES_LAT_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(CERES_LAT_data(CERES_LAT_dims(1), CERES_LAT_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, CERES_LAT_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_comp_CERES_LAT
