subroutine read_comp_TROP_SSA2(file_id)
!
! NAME:
!   read_comp_TROP_SSA2
!
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only : TROP_SSA2_dims, TROP_SSA2_data
  ...
  call h5dopen_f(file_id, 'trop_ssa2', ds_id, error)
  ...
  allocate(TROP_SSA2_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  !TROP_SSA2_dims(1) = datadims(1)
  do ii=1,ndims
    TROP_SSA2_dims(ii) = datadims(ii)
  enddo

  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(TROP_SSA2_data(TROP_SSA2_dims(1), TROP_SSA2_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, TROP_SSA2_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_comp_TROP_SSA2
