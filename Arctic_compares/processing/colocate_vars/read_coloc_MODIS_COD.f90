subroutine read_coloc_MODIS_COD(file_id)
!
! NAME:
!   read_coloc_MODIS_COD
!
! PURPOSE:
!   Read Latitude data from an HDF5 file pointed to by file_id 
!   and store the data in AI_COD (and the dimensions in AI_dims) from
!   h5_vars.
!  
! CALLS:
!   Modules:
...
!  ############################################################################
  ...
  use hdf5
  use comp_grid_vars, only : MODIS_COD_dims, MODIS_COD_data
  ...
  call h5dopen_f(file_id, 'modis_cod', ds_id, error)
  ...
  allocate(MODIS_COD_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  do ii=1,ndims
    MODIS_COD_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(MODIS_COD_data(MODIS_COD_dims(1), MODIS_COD_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, MODIS_COD_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_coloc_MODIS_COD
