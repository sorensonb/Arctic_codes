subroutine read_comp_NSIDC_LON(file_id)
!
! NAME:
!   read_comp_NSIDC_LON
!
...
!   and store the data in NSIDC_LON_data (and the dimensions in NSIDC_LON_dims) from
!   h5_vars.
!  
! CALLS:
!   Modules:
...
!  ############################################################################
  ...
  use hdf5
  use colocate_vars, only : NSIDC_LON_dims, NSIDC_LON_data
  ...
  ! Determine the number of dimensions in the dataset, allocate the NSIDC_LON_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(NSIDC_LON_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the NSIDC_LON_dims array
  do ii=1,ndims
    NSIDC_LON_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(NSIDC_LON_data(NSIDC_LON_dims(1), NSIDC_LON_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, NSIDC_LON_data, dims, &
                 error)
  if (error.lt.0) then
...
end subroutine read_comp_NSIDC_LON
