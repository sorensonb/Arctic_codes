...
int main(int argc, char *argv[]) {
    ...
    if(argc != 3)
    {
        ...
        printf("SYNTAX: ./autoceres_lwf_exec year month\n");
        return 0;
    }
    ...
    char      avg_file_start[] = "/home/bsorenson/HighLatitudeStudy/CERES/average_files/TERRA/N20to90/ceres_avg_flux_lwf_";
    ...
    // Allocate arrays
    //ceres_lwf = (float*)malloc(NLAT*sizeof(float*));
    //ceres_swf = (float*)malloc(NLAT*sizeof(float*));
    //ceres_lwf_cc = (int*)malloc(NLAT*sizeof(int*));
    //ceres_swf_cc = (int*)malloc(NLAT*sizeof(int*));
    // Initialize arrays 
    for(i=0;i<NLAT;i++)
    {
    //    ceres_lwf[i] = (float)malloc(NLON*sizeof(float));
    //    ceres_swf[i] = (float)malloc(NLON*sizeof(float));
    //    ceres_lwf_cc[i] = (int)malloc(NLON*sizeof(int));
    //    ceres_swf_cc[i] = (int)malloc(NLON*sizeof(int));
        for(j=0;j<NLON;j++)
        {
            ceres_swf[i][j]=-999.;
            ceres_swf_cc[i][j]=-9;
    ...
    while((de=readdir(dr)) != NULL)
    {
        ...
        if(slen>2)
        {
            ...
            if(strcmp(input_date,check_date)==0)
            {
                ...
                readhdf(temp_path,"CERES LW TOA flux - upwards", &llsize0,  &gain, &ofst, swf);
                ...
                //                printf("Old avg = %d %f %f\n",swf_count,tlat,avg_swf);
                                ceres_swf[lat_index][lon_index] = ((ceres_swf[lat_index][lon_index]*ceres_swf_cc[lat_index][lon_index])+
                                    swf[j])/(ceres_swf_cc[lat_index][lon_index]+1);
                ...
                //            else {
                ////                printf("Old avg = %d %f %f\n",swf_count,tlat,avg_swf);
                //                ceres_lwf[lat_index][lon_index] = ((ceres_lwf[lat_index][lon_index]*
                //                    ceres_lwf_cc[lat_index][lon_index])+lwf[j])/(ceres_lwf_cc[lat_index][lon_index]+1);
    ...
    if( fout != NULL)
    {
        for(i=(minlat+90);i<NLAT;i++)
        {
            ...
            for(j=0;j<NLON;j++)
            {
                ...
                fprintf(fout,"%d %d %f %d\n",lat_index,lon_index,ceres_swf[i][j],ceres_swf_cc[i][j]);
