subroutine read_h5_AZM(file_id,error)
!subroutine read_h5_dataset(file_id,data_path,AZM_dims,&
...
!  ############################################################################
  ...
  use hdf5
  use h5_vars, only : AZM_dims, AZM_data
  ...
  ! Determine the number of dimensions in the dataset, allocate the AZM_dims
  ! array
  ! -------------------------------------------------------------------------
  ...
  allocate(AZM_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  ! Insert important dimensions into the AZM_dims array
  do ii=1,ndims
    AZM_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(AZM_data(AZM_dims(1), AZM_dims(2)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, AZM_data, dims, &
                 error)
  if (error.lt.0) then
