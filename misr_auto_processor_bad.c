                        ...
                        if(s2len>2)
                        {
                            ...
                            if(!strcmp(strrchr(file_name, '\0') - 3, ".nc"))
                            {
                                ...
                                for(i=0;i<max_index;i++)
                                {
                                    ...
                                    //    printf("%d aod = %f\n",i,aod_raw[i]);
                                    //} 
                                    if(((aod_raw[i]>-900.0) && (aod_raw[i] < 70)) && (lat[i] > -200) && (lon[i] > -300))
                                //        (lat[i] > latmin))
                                    {
                                        lat_index = (int)lat[i]+90;
                                        //lat_index = (int)floor(lat[i]*(1./grid_resolution) - lat_gridder);
                                        lon_index = (int)lon[i]+180;
                                        printf("%f %d %f %d\n",lat[i],lat_index,lon[i],lon_index);
