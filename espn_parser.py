...
for furl in final_urls:
    ...
    for op in opponents:
        if(op not in gdict.keys()):
            gdict[op] = {}
            gdict[op]['Team'] = {}
            gdict[op]['Team']['Offense'] = {}
            gdict[op]['Team']['Offense']['Passing'] = {}
            gdict[op]['Team']['Offense']['Rushing'] = {}
            gdict[op]['Team']['Offense']['Receiving'] = {}
    ...
    gdict[opponents[0]]['Team']['Offense']['Passing'] = total_stat[-4]

    # Team 2
    beg_idx = data.index(opponents[1] + ' Passing') 
    end_idx = data.index(opponents[0] + ' Rushing') 
    total_stat = data[beg_idx : end_idx]
    gdict[opponents[1]]['Team']['Offense']['Passing'] = total_stat[-4]
    ...
    gdict[opponents[0]]['Team']['Offense']['Rushing'] = total_rushing[-4]

    # Team 2
    beg_idx = data.index(opponents[1] + ' Rushing') 
    end_idx = data.index(opponents[0] + ' Receiving') 
    total_rushing = data[beg_idx : end_idx]
    gdict[opponents[1]]['Team']['Offense']['Rushing'] = total_rushing[-4]
    ...
    total_stat = data[beg_idx : end_idx]
    gdict[opponents[0]]['Team']['Offense']['Receiving'] = total_stat[-4]

    # Team 2
    beg_idx = data.index(opponents[1] + ' Receiving') 
    end_idx = data.index(opponents[0] + ' Fumbles') 
    total_stat = data[beg_idx : end_idx]
    gdict[opponents[1]]['Team']['Offense']['Receiving'] = total_stat[-4]
