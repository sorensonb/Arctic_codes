...
def build_training_dataset(date_str):
    ...
    for ii, fname in enumerate(files):
        ...
        # ---------------------------
        OMI_data = readOMI_swath_hdf(dt_name.strftime('%Y%m%d%H%M'), \
            'control', latmin = latmin, \
            skiprows = None)
...
def plot_losses(gen_losses, disc_losses, EPOCHS, save = False):
    ...
    ax1.plot(gen_losses[:, -1], label = 'Gen')
    ax1.plot(disc_losses[:, -1], label = 'Disc')
    ax1.legend()
    ax1.set_title(str(EPOCHS) + ' Epochs') 

    if(save):
        outname = 'losses_' + str(EPOCHS) + '.png'
