...
for arg in sys.argv:
    ...
    if(arg=='climo'):
        ...
        ttype=split_arg[1]
file_path = '/home/bsorenson/HighLatitudeStudy/OMI/Updated_Data/30to90/'
# NXAR = No Xtrack, All Rows
if(ftype=='NXAR'):
    input_file = file_path+'no_xtrack_all_rows/omi_monthly_average_AI_noxtrack_allrows_newData.txt.gz'
# XAR = Xtrack, All Rows
elif(ftype=='XAR'):
    input_file = file_path+'xtrack_all_rows/omi_monthly_average_AI_xtrack_allrows_newData.txt.gz'
# XR123 = XTrack, Rows 1-23
elif(ftype=='XR123'):
    input_file = file_path+'omi_monthly_average_AI_xtrack_row023_newData.txt.gz'
else:
...
if(trend is True):
    plotOMI(AIDict,start_date,end_date,save=save_arg,trend_type=ttype,file_type=ftype,season=season_arg)
elif(pval is True):
    plotOMI_MK(AIDict,start_date,end_date,save=save_arg,file_type='XR123',season='')
else:
    plotOMI_Climo(AIDict,start_date,end_date,save=save_arg,trend_type=ttype,file_type=ftype,season=season_arg)
