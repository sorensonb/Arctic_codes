...
def read_AIRS(date_str, zoom = False):

    dt_date_str = datetime.strptime(date_str,"%Y%m%d%H%M")

    # Extract the filename given the channel
    # --------------------------------------
    filenames = plot_limits_dict[dt_date_str.strftime('%Y-%m-%d')][dt_date_str.strftime('%H%M')]['airs']
 
    # Set up arrays to hold the AIRS data
    # -----------------------------------
    num_files = len(filenames)
    tmp = np.full((45 * num_files, 30, 28), np.nan)
    ght = np.full((45 * num_files, 30, 28), np.nan)
    h2o = np.full((45 * num_files, 30, 14), np.nan)
    lat = np.full((45 * num_files, 30), np.nan)
    lon = np.full((45 * num_files, 30), np.nan)
 
    # Loop over the file (or files)
    # -----------------------------
    
    for ii, fin in enumerate(filenames):
 
        print("Reading AIRS data from ",fin)
        ...
        # -------------------------------------------------------
        tmp[(45 * ii):(45 * ii + 45),:,:] = data.variables['TAirStd'][:,:,:]
        ght[(45 * ii):(45 * ii + 45),:,:] = data.variables['GP_Height'][:,:,:]
        h2o[(45 * ii):(45 * ii + 45),:,:] = data.variables['H2OMMRStd'][:,:,:]
        lat[(45 * ii):(45 * ii + 45),:]   = data.variables['latAIRS'][:,:,1,1]
        lon[(45 * ii):(45 * ii + 45),:]   = data.variables['lonAIRS'][:,:,1,1]
    
        data.close()


    pressStd = np.array([1100.0,1000.0,925.0,850.0,700.0,600.0,500.0,400.0,\
        300.0,250.0,200.0,150.0,100.0,70.0,50.0,30.0,20.0,15.0,10.0,7.0,5.0,\
        3.0,2.0,1.5,1.0,0.5,0.2,0.1])
    pressH2O = np.array([1100.0,1000.0,925.0,850.0,700.0,600.0,500.0,400.0,\
    ...
    ght = np.ma.masked_invalid(ght)
    h2o = np.ma.masked_invalid(h2o)
    ...
    AIRS_data['ght'] = ght
    AIRS_data['H2O'] = h2o
    AIRS_data['pressStd'] = pressStd
    AIRS_data['pressH2O'] = pressH2O
    AIRS_data['lat']  = lat
    AIRS_data['lon']  = lon
    AIRS_data['cross_date']  = cross_date
    AIRS_data['cross_time']  = cross_time
    AIRS_data['file_name'] = fin.strip().split('/')[-1]
  
 
    return AIRS_data
    
def extract_prof(AIRS_data,variable,slat,slon,yaxis):
    # Determine the indices where the desired lat and lon are located
    # ---------------------------------------------------------------
    # Find the matching grid index
    a_idx = nearest_gridpoint(slat, slon, AIRS_data['lat'], AIRS_data['lon'])

    # Extract variable data
    pdata = AIRS_data[variable][a_idx[0][0], a_idx[1][0],:]

    # Depending on whether pressure or geopotential height is being plotted
    # on the y axis, set the associated axis limit and variable names.
    # ---------------------------------------------------------------------
    yval = yaxis
    if(yaxis == 'ght'):
    ...
    # Handle the case where the profile is missing at the current point
    # -----------------------------------------------------------------
    if(False not in pdata.mask):
        # For now, just increase the y dimension by 1
        print("WARNING: no profile found at",a_idx)
        a_idx[1][0] -= 1
        print("       proceeding with ",a_idx)
        pdata = AIRS_data[variable][a_idx[0][0], a_idx[1][0],:]
        if(yval[:5] != 'press'):
            pght  = AIRS_data[yval][a_idx[0][0], a_idx[1][0],:len(pdata)] \
                    / 1000.

    return pdata, pght, AIRS_data['lat'][a_idx[0][0], a_idx[1][0]], \
        AIRS_data['lon'][a_idx[0][0], a_idx[1][0]], ylabel, yrange

def plot_AIRS_prof(date_str,variable,slat,slon,yaxis = 'ght',zoom=True,save=False):

    #dt_date_str = datetime.strptime(date_str,"%Y%m%d%H%M")

    # -----------------------------------------------------------------------
    # Step 1: Call read_AIRS to read the desired AIRS data from the
    # file and put it in a dictionary
    # -----------------------------------------------------------------------
    print(date_str)
    AIRS_data = read_AIRS(date_str, zoom = zoom)

    # Use the function to extract the desired profile
    # -----------------------------------------------
    pdata, pght, alat, alon, ylabel, yrange = \
        extract_prof(AIRS_data,variable,slat,slon,yaxis)

    print("Input: ",slat,slon)
    print("AIRS:  ", alat, alon)
    ...
    ax.set_ylim(yrange)
    ax.set_xlabel(var_dict[variable]) 
    ax.set_title(AIRS_data['cross_date'] + ' ' + AIRS_data['cross_time'] + \
        '\n' + str(alat) + ' N , ' + \
        str(alon) + ' E')
    plt.show() 

# Make a 3-panel image containing a map of near-surface temperatures,
def plot_AIRS_station_profs(date_str,variable,yaxis = 'ght',level=4,zoom=True,\
        save=False):
    ...
    # -----------------------------------------------------------------------
    # Step 2: Read in the station data for the current case to identify 
    # the station locations.
    # -----------------------------------------------------------------------
    #asos_file = plot_limits_dict[MODIS_data['cross_date']][MODIS_data['file_time']]['asos']
    asos_file = plot_limits_dict[dt_date_str.strftime('%Y-%m-%d')][dt_date_str.strftime('%H%M')]['asos']
    df = pd.read_csv(asos_file)

    # -----------------------------------------------------------------------
    # Step 3: Plot a map of near-surface temperatures
    # -----------------------------------------------------------------------
    plt.close('all')
    fig = plt.figure(figsize=(22,5))
    ax0 = fig.add_subplot(1,3,3,projection = ccrs.LambertConformal())
    ax1 = fig.add_subplot(1,3,1)
    ax2 = fig.add_subplot(1,3,2)

    ##!#local_tmp = np.copy(AIRS_data['tmp'][:,:,4])
    ##!#local_tmp[(((AIRS_data['lat'] < plot_limits_dict[dt_date_str.strftime('%Y-%m-%d')][dt_date_str.strftime('%H%M')]['Lat'][0]) | \
    ##!#            (AIRS_data['lat'] > plot_limits_dict[dt_date_str.strftime('%Y-%m-%d')][dt_date_str.strftime('%H%M')]['Lat'][1])) | \
    ##!#           ((AIRS_data['lon'] < plot_limits_dict[dt_date_str.strftime('%Y-%m-%d')][dt_date_str.strftime('%H%M')]['Lon'][0]) | \
    ##!#            (AIRS_data['lon'] > plot_limits_dict[dt_date_str.strftime('%Y-%m-%d')][dt_date_str.strftime('%H%M')]['Lon'][1])))] = np.nan
    ##!#local_tmp = np.ma.masked_invalid(local_tmp)
    ##!#print(np.nanmax(local_tmp), np.nanmin(local_tmp))
    ...
    if(zoom):
        ax0.set_extent([plot_limits_dict[dt_date_str.strftime('%Y-%m-%d')][dt_date_str.strftime('%H%M')]['Lon'][0], \
                        ...
                        plot_limits_dict[dt_date_str.strftime('%Y-%m-%d')][dt_date_str.strftime('%H%M')]['Lat'][0], \
                        plot_limits_dict[dt_date_str.strftime('%Y-%m-%d')][dt_date_str.strftime('%H%M')]['Lat'][1]], \
                        datacrs)

    # -----------------------------------------------------------------------
    # Step 3: For each station, extract a profile for that location and plot
    # the profile on the figure.
    # -----------------------------------------------------------------------
    
    # Extract the unique station names from the asos file
    station_names = sorted(set(df['station'].values))
    for ii, station in enumerate(station_names):
        print(station)
        ...
        # -----------------------------------------------
        pdata, pght, alat, alon, ylabel, yrange = \
            extract_prof(AIRS_data,variable,lat_stn,lon_stn,yaxis)

        ax0.text(lon_stn, lat_stn, station, fontsize=10,weight='bold', \
            transform=datacrs, color='lime')

        print("Input: ",lat_stn,lon_stn)
        print("AIRS:  ", alat, alon)
        
        # Plot the data on the figure
        ax1.plot(pdata, pght, label=station)

        # For the third panel, pull the smokey station data for subtraction
        if(station != case_dict[date_str]):
            lat_stn = df['lat'][df['station'] == case_dict[date_str]].values[0]
            lon_stn = df['lon'][df['station'] == case_dict[date_str]].values[0]

            # Use the function to extract the desired profile
            # -----------------------------------------------
            pdata_ref, pght_ref, alat, alon, ylabel, yrange = \
                extract_prof(AIRS_data,variable,lat_stn,lon_stn,yaxis)

            diff_data = pdata_ref - pdata

            # Plot the data on the figure
            ax2.plot(diff_data, pght, label=station)
    

    # Plot a 0 line
    # -------------
    ax2.plot(np.zeros(int(yrange[1]-yrange[0]+1)),np.arange(yrange[0],yrange[1]+1),'--',color='black')
...
# Make a 3-panel image containing a map of near-surface temperatures,
def compare_AIRS_profs(date_str1,date_str2,variable,yaxis = 'ght',level=4,\
    ...
    # -----------------------------------------------------------------------
    #asos_file = plot_limits_dict[MODIS_data['cross_date']][MODIS_data['file_time']]['asos']
    asos_file = plot_limits_dict[dt_date_str1.strftime('%Y-%m-%d')][dt_date_str1.strftime('%H%M')]['asos']
    df = pd.read_csv(asos_file)
    ...
    AIRS_data1 = read_AIRS(date_str1, zoom = zoom)
    AIRS_data2 = read_AIRS(date_str2, zoom = zoom)

    # -----------------------------------------------------------------------
    # Step 4: For each station, extract a profile for that location and plot
    # the profile on the figure.
    # -----------------------------------------------------------------------
    ...
    for ii, station in enumerate(station_names):
        ...
        # Use the function to extract the desired profile from date 1
        # -----------------------------------------------------------
        pdata1, pght1, alat1, alon1, ylabel, yrange = \
            extract_prof(AIRS_data1,variable,lat_stn,lon_stn,yaxis)

        # Use the function to extract the desired profile from date 2
        # -----------------------------------------------------------
        pdata2, pght2, alat2, alon2, ylabel, yrange = \
            extract_prof(AIRS_data2,variable,lat_stn,lon_stn,yaxis)

        print("Input: ",lat_stn,lon_stn)
        print("AIRS1:  ", alat1, alon1)
        print("AIRS2:  ", alat2, alon2)
       
        # Calculate difference between date 2 and date 1 data
        # ---------------------------------------------------
        diff_data = pdata2 - pdata1
 
        # Plot the data on the figure
        ax0.plot(pdata1, pght1, label = station + ' old', color = colors[ii])
        ax0.plot(pdata2, pght2, '--', label = station + ' new', color = colors[ii])
        ax1.plot(diff_data, pght1, label=station, color=colors[ii])
    ...
    ax0.set_ylim(yrange)
    ax0.set_xlabel(var_dict[variable]) 
    ax0.legend()

    ax1.set_title('AIRS Profile Differences\n' + \
        dt_date_str1.strftime('%Y%m%d%H%M') + ' - ' + \
        dt_date_str2.strftime('%Y%m%d%H%M'))
    ax1.set_ylabel(ylabel)
    ax1.set_ylim(yrange)
