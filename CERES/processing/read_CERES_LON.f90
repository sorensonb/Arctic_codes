subroutine read_CERES_LON(file_id)
!
! NAME:
!   read_CERES_LON
!
...
!  ############################################################################
  ...
  use hdf5
  use ceres_vars, only : CERES_LON_dims, CERES_LON_data
  ...
  ! ----------------------------------------------------------------------
  allocate(CERES_LON_data(CERES_LON_dims(1)), stat=error)
  if ( error < 0 ) then
