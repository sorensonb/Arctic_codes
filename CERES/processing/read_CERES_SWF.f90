subroutine read_CERES_SWF(file_id)
!
! NAME:
!   read_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use ceres_vars, only : CERES_SWF_dims, CERES_SWF_data
  ...
  ! ----------------------------------------------------------------------
  allocate(CERES_SWF_data(CERES_SWF_dims(1)), stat=error)
  if ( error < 0 ) then
