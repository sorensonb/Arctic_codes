subroutine read_CERES_LAT(file_id)
!
! NAME:
!   read_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use ceres_vars, only : CERES_LAT_dims, CERES_LAT_data
  ...
  ! ----------------------------------------------------------------------
  allocate(CERES_LAT_data(CERES_LAT_dims(1)), stat=error)
  if ( error < 0 ) then
