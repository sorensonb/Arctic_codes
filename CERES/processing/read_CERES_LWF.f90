subroutine read_CERES_LWF(file_id)
!
! NAME:
!   read_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use ceres_vars, only : CERES_LWF_dims, CERES_LWF_data
  ...
  ! ----------------------------------------------------------------------
  allocate(CERES_LWF_data(CERES_LWF_dims(1)), stat=error)
  if ( error < 0 ) then
