subroutine read_CERES_ALB(file_id)
!
! NAME:
!   read_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use ceres_vars, only : CERES_ALB_dims, CERES_ALB_data
  ...
  ! ----------------------------------------------------------------------
  allocate(CERES_ALB_data(CERES_ALB_dims(1)), stat=error)
  if ( error < 0 ) then
