subroutine read_CERES_SZA(file_id)
!
! NAME:
!   read_AI
!
...
!  ############################################################################
  ...
  use hdf5
  use ceres_vars, only : CERES_SZA_dims, CERES_SZA_data
  ...
  allocate(CERES_SZA_dims(ndims),stat=error)
  if ( error /= 0 ) then
  ...
  do ii=1,ndims
    CERES_SZA_dims(ii) = datadims(ii)
  enddo
  
  ! Read the dataset and transfer the result to an allocated working array
  ! ----------------------------------------------------------------------
  allocate(CERES_SZA_data(CERES_SZA_dims(1)), stat=error)
  if ( error < 0 ) then
     write(*,*) " *** Error allocating H5dataset"
     return
  endif

  call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, CERES_SZA_data, dims, &
                 error)
  if (error.lt.0) then
